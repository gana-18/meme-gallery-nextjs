...
function MemeCard() {
  const [memes, setMemes] = useState([]);
    ...
    const fetchMemes = async () => {
        try {
            const response = await fetch(`https://www.reddit.com/r/memes.json?limit=100${after ? `&after=${after}` : ''}`);
...
/* function MemeCard(){
    ...
    const fetchMemes = async () => {
        const response = await fetch('https://www.reddit.com/r/memes.json?limit=1000');
        const json = await response.json();
        setMemes(json.data.children);
        setLoading(false)
    }
    useEffect(() => {
        fetchMemes();
    }, []);
    memes?memes.shift():console.log("Loading");
    return (
        <>
        {loading?(
            <LoadingPage/>
